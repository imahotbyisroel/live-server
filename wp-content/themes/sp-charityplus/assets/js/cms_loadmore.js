	...
	$('.cms-grid-wraper').each(function(){
		...
		if(typeof cms_variable != 'undefined'){
			...
			$.loadData = function(html_id){
				...
				$.get(nextLink[html_id],function(data){
					...
					if(pageNum[html_id] <= max[html_id]) {
						...
						$('#'+html_id +' .cms-load-posts a').html('No more posts to load');
					}
			...
			if(pageNum[html_id] <= max[html_id]) {
				...
				$('#'+html_id +' .cms_pagination').append('<div class="cms-placeholder-'+ pageNum[html_id] +'"></div><div class="cms-load-posts text-center"><a data-loading="0" href="#" class="btn">'+text+'</a></div>');
			}
