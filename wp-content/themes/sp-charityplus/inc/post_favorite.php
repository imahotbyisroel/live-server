...
class NectarLove {
	...
	function ajax($post_id) {
		...
		if( isset($_POST['loves_id']) ) {
			...
			echo esc_attr( $this->love_post($post_id, 'update') );
		}
		//get
		else {
			$post_id = str_replace('nectar-love-', '', $_POST['loves_id']);
			echo esc_attr( $this->love_post($post_id, 'get') );
		}
		exit;
	}
	...
	function love_post($post_id, $action = 'get')
	{
		...
		$unit = esc_html__('likes','sp-charityplus');
		switch($action) {
	...
	function add_love($unit = '', $show_icon = true) {
		...
		$output = $this->love_post($post->ID);
  		...
  		$class = 'nectar-love';
  		$icon = 'fa fa-heart-o';
  		$title = esc_html__('Like this', 'sp-charityplus');
		if( isset($_COOKIE['nectar_love_'. $post->ID]) ){
			$class = 'nectar-love loved unselected';
			$icon = 'fa fa-heart';
			$title = esc_html__('You already liked this!', 'sp-charityplus');
		}
		$heart_icon = '';
		if($show_icon){
			$heart_icon = '<i class="'.$icon.'"></i>';
		}
		return '<a href="#" class="'. $class .'" id="nectar-love-'. $post->ID .'" title="'. $title .'"> '.$heart_icon. ' <span class="nectar-love-count">' . $output .' '.$unit.'</span></a>';
	}
