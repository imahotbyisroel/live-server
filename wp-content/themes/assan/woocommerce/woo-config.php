...
function crazy_assan_header_mini_cart_function($menu, $args) {
    ...
    if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins'))) && ($args->menu == 'primary_nav')) {
        $menu.='<li><a class="cart-contents" href="' . $link . '"><i class="fa fa-shopping-cart"></i> (' . $woocommerce->cart->cart_contents_count . ')</a></li>';
    }
    return $menu;
}
...
function woocommerce_header_add_to_cart_fragment($fragments) {
    ...
    $link = function_exists('wc_get_cart_url') ? wc_get_cart_url() : $woocommerce->cart->get_cart_url();
    ?>
    <a class="cart-contents" href="<?php echo $link; ?>"><i class="fa fa-shopping-cart"></i> (<?php echo $woocommerce->cart->cart_contents_count; ?>)</a> 
    <?php
