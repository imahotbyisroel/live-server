...
function crazy_assan_scripts() {
    ...
    // WooCommerce
    if (function_exists("is_woocommerce")) {
        wp_enqueue_script('assan-woo-custom-script', ASSAN_SKIN_PATH . 'js/woo-custom-scripts.js', array('jquery'), '', TRUE);
    }
...
function crazy_assan_import_demo_files() {
    return array(
        array(
            'import_file_name' => 'Assan Demo Import',
            'local_import_file' => trailingslashit(get_template_directory()) . 'lib/sample-data/assan-demo-content.xml',
            'local_import_widget_file' => trailingslashit(get_template_directory()) . 'lib/sample-data/assan-demo-widgets.wie',
            'import_notice' => __('After you import this demo, you will have to setup the slider separately.', 'assan'),
        ),
        array(
            'import_file_name' => 'WooCommerce Product Import',
            'local_import_file' => trailingslashit(ABSPATH) . 'wp-content/plugins/woocommerce/dummy-data/dummy-data.xml',
        )
    );
}

add_filter('pt-ocdi/import_files', 'crazy_assan_import_demo_files');

function crazy_assan_after_import_setup() {
    // Assign menus to their locations.
    $main_menu = get_term_by('name', 'Main Menu', 'nav_menu');
    $onepage_menu = get_term_by('name', 'One page', 'nav_menu');

    set_theme_mod('nav_menu_locations', array(
        'primary_nav' => $main_menu->term_id,
        'onepage_nav' => $onepage_menu->term_id
            )
    );

    // Assign front page and posts page (blog page).
    $front_page_id = get_page_by_title('Home Flex Slider');
...
add_action('pt-ocdi/after_import', 'crazy_assan_after_import_setup');
